...
Phase 7 implements the fixed-size `Page` abstraction, file-backed `DiskManager`, bounded `BufferPoolManager`, `LRUReplacer`, the record layer, slotted `TablePage`, `TableHeap`, a persistent catalog, the SQL lexer, and a typed recursive-descent parser with AST nodes. Query planning and execution are not implemented yet.
